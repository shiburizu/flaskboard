...
def showthread(ident,b):
	try:
		op = g.db.execute("SELECT name,post,id FROM threads WHERE id = %d AND board = %s", (ident,b)).fetchall()[0]
		print(op)
		posts = g.db.execute("SELECT * FROM posts WHERE parent = %d AND board = %s" % (ident,b)).fetchall()
		title = g.db.execute("SELECT name FROM threads WHERE id = %d AND board = %s" % (ident,b)).fetchall()[0][0]
...
def post(b):
	...
	try:
		...
		id = 0
	print(id+1)
	g.db.execute("INSERT INTO threads VALUES('%s','%s',%d,'%s')" % (name,comment,int(id+1),str(b)))
...
def postreply(b,ident):
	...
	try:
		...
		id = 0
	print(id+1)
	g.db.execute("INSERT INTO posts VALUES('%s','%s',%d,'%s','%s')" % (name,comment,int(id+1),str(b),str(ident)))
	g.db.execute("UPDATE boards SET postcount = postcount + 1 WHERE name = '%s'" % b)
	#g.db.commit()
